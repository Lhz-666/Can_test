...
//		bopan_3508.Set_position=(channels[3]-1000)*360/1000;//这里的通道尚未确定，先给为3通道	
		...
		bopan_3508.position=bopan_GetAngle;//yaw外环（位置环）
		...
		inner_bopan_3508.speed=bopan_GetSpeed;//yaw内环（速度环）
...
void StartTask02(void const * argument)//打印并关电时间
{
...
//		printf("Rpm:%d,Speed:%d,Current:%d,Temperature:%d\n", GetRpm,GetSpeed,GetCurrent,GetTemperature);  // 两个 %d，两个参数
		printf("%d,%d,%d,%d,%d,%d\n", yaw_GetSpeed,inner_yaw_setposition,(int)bopan_3508.position,(int)bopan_3508.Set_position,(int)inner_bopan_3508.speed,(int)inner_bopan_3508.Set_speed);  //打印
